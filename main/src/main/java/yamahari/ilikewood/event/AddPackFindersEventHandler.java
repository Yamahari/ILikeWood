        ...
        for (final IModPlugin plugin : ILikeWood.PLUGINS)
        {
            ...
            ILikeWoodConfig.getAll().forEach(config ->
            {
                if (config.isEnabled())
                {
                    ...
                    final var pack = new PathPackResources(pluginModFile.getFileName() + ":" + resourcePath,
                        resourcePath
                    );
                    ...
                    {
                        event.addRepositorySource((consumer, constructor) -> consumer.accept(constructor.create(String.format("%s:%s-%s",
                                Constants.MOD_ID,
                                config.name(),
                                modId
                            ),
                            Component.translatable(String.format("%s - %s - %s resources",
                                Constants.MOD_ID,
                                config.name(),
                                modId
                            )),
