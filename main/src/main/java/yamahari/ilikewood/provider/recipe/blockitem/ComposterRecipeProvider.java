...
public final class ComposterRecipeProvider extends AbstractBlockItemRecipeProvider
{
    ...
    )
    {
        ...
        ShapedRecipeBuilder.shaped(block).define('#', slab).pattern("# #").pattern("# #").pattern("###").unlockedBy(
            "has_panels",
            has(slab)
        ).group(String.format("%s:%s", Constants.MOD_ID, Constants.COMPOSTER_PLURAL)).save(consumer,
