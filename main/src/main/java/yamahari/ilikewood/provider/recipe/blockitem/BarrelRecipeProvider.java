...
public final class BarrelRecipeProvider extends AbstractBlockItemRecipeProvider
{
    ...
    )
    {
        ...
        ShapedRecipeBuilder.shaped(block).define('P', panels).define('S', slab).pattern("PSP").pattern("P P").pattern(
            "PSP").unlockedBy("has_panels", has(panels)).group(String.format("%s:%s",
            Constants.MOD_ID,
