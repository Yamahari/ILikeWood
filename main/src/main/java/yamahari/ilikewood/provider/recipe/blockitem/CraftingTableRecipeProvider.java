...
public final class CraftingTableRecipeProvider extends AbstractBlockItemRecipeProvider
{
    ...
    )
    {
        ...
        ShapedRecipeBuilder.shaped(block).define('#', panels).pattern("##").pattern("##").unlockedBy(
            "has_panels",
            has(panels)
        ).group(String.format("%s:%s", Constants.MOD_ID, Constants.CRAFTING_TABLE_PLURAL)).save(
            consumer,
