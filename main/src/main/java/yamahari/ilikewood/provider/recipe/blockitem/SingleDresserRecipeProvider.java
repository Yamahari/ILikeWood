...
public final class SingleDresserRecipeProvider extends AbstractBlockItemRecipeProvider
{
    ...
    )
    {
        ...
        final ItemLike log = ForgeRegistries.BLOCKS.getValue(ILikeWood.WOODEN_RESOURCE_REGISTRY.getLog(woodType)
            .getResource());

        ShapedRecipeBuilder.shaped(block, 3).define('#', Objects.requireNonNull(log)).define('X',
            Objects.requireNonNull(strippedLog)
        ).pattern("###").pattern("XXX").pattern("XXX").group(String.format("%s:%s",
            Constants.MOD_ID,
