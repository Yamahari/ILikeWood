...
public final class TorchRecipeProvider extends AbstractBlockItemRecipeProvider
{
    ...
    )
    {
        ...
        ShapedRecipeBuilder.shaped(block, 4).define('I', stick).define('#', coals).pattern("#").pattern("I").unlockedBy(
            "has_coal",
            has(ItemTags.COALS)
        ).group(String.format("%s:%s", Constants.MOD_ID, Constants.TORCH_PLURAL)).save(
            consumer,
