    ...
    public static void onRegisterAdditionalModel(final ModelEvent.RegisterAdditional event)
    {
        ...
        if (ILikeWoodConfig.ITEM_FRAMES_CONFIG.isEnabled())
        {
            ILikeWood.WOOD_TYPE_REGISTRY.getWoodTypes().filter(woodType -> woodType.getItemTypes().contains(WoodenItemType.ITEM_FRAME)).forEach(woodType ->
            {
                ...
                itemFrameModels.put(woodType, new ResourceLocation(Constants.MOD_ID, Util.toPath(path, woodType.getModId(), woodType.getName())));
                itemFrameMapModels.put(woodType, new ResourceLocation(Constants.MOD_ID, Util.toPath(path, woodType.getModId(), "map", woodType.getName())));
            });
        }
        ...
        Stream.of(ITEM_FRAME_MAP_MODELS.entrySet(), ITEM_FRAME_MODELS.entrySet()).flatMap(Collection::stream).map(Map.Entry::getValue).forEach(event::register);
    }
}
