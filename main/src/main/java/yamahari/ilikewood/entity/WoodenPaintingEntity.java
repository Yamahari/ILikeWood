...
public final class WoodenPaintingEntity
    ...
    implements IWooden
{
    private static final EntityDataAccessor<String> DATA_WOOD_TYPE_RESOURCE_LOCATION =
        SynchedEntityData.defineId(WoodenPaintingEntity.class, EntityDataSerializers.STRING);
    private IWoodType woodType;

    public WoodenPaintingEntity(
        final EntityType<? extends WoodenPaintingEntity> entityType,
        final Level level
    )
    {
    ...
    public static Optional<WoodenPaintingEntity> create(
        final Level level,
        ...
        if (hand == InteractionHand.MAIN_HAND && !player.isSecondaryUseActive())
        {
            ...
            if (!itemInHand.isEmpty())
            {
                if (itemInHand.getItem() instanceof WoodenPaintingItem)
                {
                    ...
                    final List<Holder<PaintingVariant>> list = new ArrayList<>();
                    Registry.PAINTING_VARIANT.getTagOrEmpty(PaintingVariantTags.PLACEABLE).forEach(variant ->
                    {
                        if (currentVariant != variant && variant.get().getWidth() == currentVariant.get().getWidth() && variant.get().getHeight() == currentVariant
                            .get()
                            .getHeight())
                        {
                            list.add(variant);
