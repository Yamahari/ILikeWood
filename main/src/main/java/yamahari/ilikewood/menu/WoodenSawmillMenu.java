    ...
    extends AbstractContainerMenu
{
    ...
    private void setupRecipeList(final Container inventory, final ItemStack stack) {
        ...
        if (!stack.isEmpty())
        {
            this.recipes = this.world.getRecipeManager()
                .getRecipesFor((RecipeType<AbstractWoodenSawmillRecipe>) WoodenRecipeTypes.SAWMILLING.get(),
                    inventory,
                    this.world
                );
        }
    ...
    public ItemStack quickMoveStack(@Nonnull final Player playerIn, final int index) {
        ...
        if (slot != null && slot.hasItem())
        {
            ...
            else if (index == 0)
            {
                ...
                .getRecipeFor((RecipeType<AbstractWoodenSawmillRecipe>) WoodenRecipeTypes.SAWMILLING.get(),
                    new SimpleContainer(stackInSlot),
                    this.world
                )
                .isPresent())
            {
