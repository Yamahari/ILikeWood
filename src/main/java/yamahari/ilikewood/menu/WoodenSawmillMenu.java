...
public final class WoodenSawmillMenu extends AbstractContainerMenu {
    ...
    public WoodenSawmillMenu(final int windowId, final Inventory playerInventory) {
        this(windowId, playerInventory, ContainerLevelAccess.NULL);
    }

    public WoodenSawmillMenu(final int windowId, final Inventory playerInventory, final ContainerLevelAccess worldPosCallable) {
    ...
    public boolean stillValid(@Nonnull final Player player) {
        return this.worldPosCallable.evaluate((world, blockPos) -> {
            ...
            return ILikeWood.BLOCK_REGISTRY.getObjects(WoodenBlockType.SAWMILL).anyMatch(blockState::is) && player.distanceToSqr((double) blockPos.getX() + 0.5D,
    ...
    private void setupRecipeList(final Container inventory, final ItemStack stack) {
        ...
        if (!stack.isEmpty()) {
            this.recipes = this.world.getRecipeManager().getRecipesFor((RecipeType<AbstractWoodenSawmillRecipe>) WoodenRecipeTypes.SAWMILLING.get(), inventory, this.world);
        }
    }
    ...
    public ItemStack quickMoveStack(@Nonnull final Player playerIn, final int index) {
        ...
        if (slot != null && slot.hasItem()) {
            ...
            if (index == 1) {
                ...
                .getRecipeFor((RecipeType<AbstractWoodenSawmillRecipe>) WoodenRecipeTypes.SAWMILLING.get(), new SimpleContainer(stackInSlot), this.world)
                .isPresent()) {
    ...
    private static final class WoodenSawmillContainerOutputSlot extends Slot {
        ...
        public WoodenSawmillContainerOutputSlot(final Container inventoryIn, final int index, final int xPosition, final int yPosition, final WoodenSawmillMenu container) {
        ...
        public void onTake(@Nonnull final Player player, final ItemStack stack) {
            ...
            this.container.inventory.awardUsedRecipes(player, List.of(this.container.inputInventorySlot.getItem()));
