...
public final class BowItemModelProvider extends AbstractItemModelProvider {
    public BowItemModelProvider(final DataGenerator generator, final ExistingFileHelper helper) {
        super(generator, helper, WoodenObjectTypes.BOW);
    }
    ...
    protected void registerModel(final Item item) {
        ...
        this
            ...
            .texture("layer0", modLoc(Util.toPath(ITEM_FOLDER, WoodenObjectTypes.BOW.getName(), woodType)))
            .transforms()
            ...
            .model(new ModelFile.UncheckedModelFile(modLoc(Util.toPath(ITEM_FOLDER,
                Util.toRegistryName(woodType, WoodenObjectTypes.BOW.getName(), "pulling", "0")))))
            .end()
            .override()
            .predicate(mcLoc("pulling"), 1.0F)
            .predicate(mcLoc("pull"), 0.65F)
            .model(new ModelFile.UncheckedModelFile(modLoc(Util.toPath(ITEM_FOLDER,
                Util.toRegistryName(woodType, WoodenObjectTypes.BOW.getName(), "pulling", "1")))))
            .end()
            .override()
            .predicate(mcLoc("pulling"), 1.0F)
            .predicate(mcLoc("pull"), 0.9F)
            .model(new ModelFile.UncheckedModelFile(modLoc(Util.toPath(ITEM_FOLDER,
                Util.toRegistryName(woodType, WoodenObjectTypes.BOW.getName(), "pulling", "2")))))
            .end();
