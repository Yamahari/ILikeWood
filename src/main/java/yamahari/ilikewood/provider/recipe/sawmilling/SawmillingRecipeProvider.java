...
public final class SawmillingRecipeProvider extends RecipeProvider {
    ...
    private static SingleItemRecipeBuilder sawmillingRecipe(final Ingredient ingredient, final ItemLike result, int count) {
        // TODO RecipeCategory
        return new SingleItemRecipeBuilder(RecipeCategory.MISC, WoodenRecipeSerializers.SAWMILLING.get(), ingredient, result, count);
    }

    private static void panelsRecipe(final Consumer<FinishedRecipe> consumer, final IWoodType woodType, final Block block, final int count) {
    ...
    private static void panelsSlabRecipe(final Consumer<FinishedRecipe> consumer, final IWoodType woodType, final Block block, final int count, @Nullable final ICondition condition) {
        if (woodType.getBlockTypes().contains(WoodenBlockType.PANELS_SLAB) || woodType.getBuiltinBlockTypes().contains(WoodenBlockType.PANELS_SLAB)) {
            ...
            final RecipeBuilder builder = sawmillingRecipe(Ingredient.of(block), panelsSlab, count
            ).unlockedBy(String.format("has_%s", blockPath), has(block));
            final var conditionalBuilder = ConditionalRecipe.builder().addCondition(new ConfigCondition(ILikeWoodConfig.PANELS_SLABS_CONFIG.name()));
            if (condition != null) {
                conditionalBuilder.addCondition(condition);
            }
            conditionalBuilder.addRecipe(builder::save).build(consumer, new ResourceLocation(Constants.MOD_ID, Util.toRegistryName(panelsSlabPath, "from", blockPath, "sawmilling"))
            );
        }
    }

    private static void panelsStairsRecipe(final Consumer<FinishedRecipe> consumer, final IWoodType woodType, final Block block, final int count, @Nullable final ICondition condition) {
        if (woodType.getBlockTypes().contains(WoodenBlockType.PANELS_STAIRS) || woodType.getBuiltinBlockTypes().contains(WoodenBlockType.PANELS_STAIRS)) {
            ...
            final RecipeBuilder builder = sawmillingRecipe(Ingredient.of(block), panelsStairs, count
            ).unlockedBy(String.format("has_%s", blockPath), has(block));
            final var conditionalBuilder = ConditionalRecipe.builder().addCondition(new ConfigCondition(ILikeWoodConfig.PANELS_STAIRS_CONFIG.name()));
            if (condition != null) {
                conditionalBuilder.addCondition(condition);
            }
            conditionalBuilder.addRecipe(builder::save).build(consumer,
                new ResourceLocation(Constants.MOD_ID, Util.toRegistryName(panelsStairsPath, "from", blockPath, "sawmilling"))
            );
        }
    }

    private static void stickRecipe(final Consumer<FinishedRecipe> consumer, final IWoodType woodType, final Block block, final int count, @Nullable final ICondition condition) {
        if (woodType.getItemTypes().contains(WoodenItemType.STICK) || woodType.getBuiltinItemTypes().contains(WoodenItemType.STICK)) {
            ...
            final var builder = sawmillingRecipe(Ingredient.of(block), stick, count).unlockedBy(String.format("has_%s", blockPath
            ), has(block));
            final var conditionalBuilder = ConditionalRecipe.builder().addCondition(new ConfigCondition(ILikeWoodConfig.STICKS_CONFIG.name()));
            if (condition != null) {
                conditionalBuilder.addCondition(condition);
            }
            conditionalBuilder.addRecipe(builder::save).build(consumer, new ResourceLocation(Constants.MOD_ID, Util.toRegistryName(stickPath, "from", blockPath, "sawmilling"))
            );
        }
    }

    private static void wallRecipe(final Consumer<FinishedRecipe> consumer, final IWoodType woodType, final Block block, final int count) {
        if (woodType.getBlockTypes().contains(WoodenBlockType.WALL) || woodType.getBuiltinBlockTypes().contains(WoodenBlockType.WALL)) {
            ...
            final RecipeBuilder builder = sawmillingRecipe(Ingredient.of(block), wall, count).unlockedBy(String.format("has_%s", blockPath), has(block));
            ConditionalRecipe.builder()
                .addCondition(new ConfigCondition(ILikeWoodConfig.WALLS_CONFIG.name()))
                .addRecipe(builder::save).build(consumer, new ResourceLocation(Constants.MOD_ID, Util.toRegistryName(wallPath, "from", blockPath, "sawmilling")));
        }
    }

    private static void postRecipe(final Consumer<FinishedRecipe> consumer, final IWoodType woodType, final Block block, final int count) {
        if (woodType.getBlockTypes().contains(WoodenBlockType.POST) || woodType.getBuiltinBlockTypes().contains(WoodenBlockType.POST)) {
            ...
            final RecipeBuilder builder = sawmillingRecipe(Ingredient.of(block), post, count).unlockedBy(String.format("has_%s", blockPath), has(block));
            ConditionalRecipe.builder()
                .addCondition(new ConfigCondition(ILikeWoodConfig.POSTS_CONFIG.name()))
                .addRecipe(builder::save).build(consumer, new ResourceLocation(Constants.MOD_ID, Util.toRegistryName(postPath, "from", blockPath, "sawmilling")));
        }
    }

    private static void strippedPostRecipe(final Consumer<FinishedRecipe> consumer, final IWoodType woodType, final Block block, final int count) {
        if (woodType.getBlockTypes().contains(WoodenBlockType.STRIPPED_POST) || woodType.getBuiltinBlockTypes().contains(WoodenBlockType.STRIPPED_POST)) {
            ...
            final RecipeBuilder builder = sawmillingRecipe(Ingredient.of(block), strippedPost, count
            ).unlockedBy(String.format("has_%s", blockPath), has(block));
            ConditionalRecipe.builder()
                .addCondition(new ConfigCondition(ILikeWoodConfig.POSTS_CONFIG.name()))
                .addRecipe(builder::save).build(consumer, new ResourceLocation(Constants.MOD_ID, Util.toRegistryName(strippedPostPath, "from", blockPath, "sawmilling")));
        }
    }

    private static void logPileRecipe(final Consumer<FinishedRecipe> consumer, final IWoodType woodType, final Block block, final int count) {
        if (woodType.getBlockTypes().contains(WoodenBlockType.LOG_PILE) || woodType.getBuiltinBlockTypes().contains(WoodenBlockType.LOG_PILE)) {
            ...
            final RecipeBuilder builder = sawmillingRecipe(Ingredient.of(block), logPile, count
            ).unlockedBy(String.format("has_%s", blockPath), has(block));
            ConditionalRecipe.builder()
                .addCondition(new ConfigCondition(ILikeWoodConfig.LOG_PILE_CONFIG.name()))
                .addRecipe(builder::save).build(consumer, new ResourceLocation(Constants.MOD_ID, Util.toRegistryName(logPilePath, "from", blockPath, "sawmilling")));
        }
    }

    private static void planksRecipe(final Consumer<FinishedRecipe> consumer, final IWoodType woodType, final Block block, final int count, @Nullable final ICondition condition) {
        if (ILikeWood.WOODEN_RESOURCE_REGISTRY.hasPlanks(woodType)) {
            final var blockPath = Objects.requireNonNull(ForgeRegistries.BLOCKS.getKey(block)).getPath();
            final var planks = Objects.requireNonNull(ForgeRegistries.BLOCKS.getValue(ILikeWood.WOODEN_RESOURCE_REGISTRY.getPlanks(woodType).getResource()));
            final var planksPath = Objects.requireNonNull(ForgeRegistries.BLOCKS.getKey(planks)).getPath();
            final var builder = sawmillingRecipe(Ingredient.of(block), planks, count).unlockedBy(String.format("has_%s", blockPath
            ), has(block));
            if (condition == null) {
                builder.save(consumer, new ResourceLocation(Constants.MOD_ID, Util.toRegistryName(planksPath, "from", blockPath, "sawmilling"))
                );
            }
            else {
                ConditionalRecipe.builder().addCondition(condition).addRecipe(builder::save).build(consumer,
                    new ResourceLocation(Constants.MOD_ID, Util.toRegistryName(planksPath, "from", blockPath, "sawmilling"))
                );
            }
        }
    }

    private static void slabRecipe(final Consumer<FinishedRecipe> consumer, final IWoodType woodType, final Block block, final int count, @Nullable final ICondition condition) {
        if (ILikeWood.WOODEN_RESOURCE_REGISTRY.hasSlab(woodType)) {
            final var blockPath = Objects.requireNonNull(ForgeRegistries.BLOCKS.getKey(block)).getPath();
            final var slab = Objects.requireNonNull(ForgeRegistries.BLOCKS.getValue(ILikeWood.WOODEN_RESOURCE_REGISTRY.getSlab(woodType).getResource()));
            final var slabPath = Objects.requireNonNull(ForgeRegistries.BLOCKS.getKey(slab)).getPath();
            final var builder = sawmillingRecipe(Ingredient.of(block), slab, count).unlockedBy(String.format("has_%s", blockPath
            ), has(block));
            if (condition == null) {
                builder.save(consumer, new ResourceLocation(Constants.MOD_ID, Util.toRegistryName(slabPath, "from", blockPath, "sawmilling"))
                );
            }
            else {
                ConditionalRecipe.builder().addCondition(condition).addRecipe(builder::save).build(consumer,
                    new ResourceLocation(Constants.MOD_ID, Util.toRegistryName(slabPath, "from", blockPath, "sawmilling"))
                );
            }
        }
    }

    private static void vanillaStickRecipe(final Consumer<FinishedRecipe> consumer, final Block block, final int count, @Nullable final ICondition condition) {
        final var blockPath = Objects.requireNonNull(ForgeRegistries.BLOCKS.getKey(block)).getPath();
        final var stickPath = Objects.requireNonNull(ForgeRegistries.ITEMS.getKey(Items.STICK)).getPath();
        final RecipeBuilder builder = sawmillingRecipe(Ingredient.of(block), Items.STICK, count
        ).unlockedBy(String.format("has_%s", blockPath), has(block));
        if (condition == null) {
            builder.save(consumer, new ResourceLocation(Constants.MOD_ID, Util.toRegistryName(stickPath, "from", blockPath, "sawmilling"))
            );
        }
        else {
            ConditionalRecipe.builder().addCondition(condition).addRecipe(builder::save).build(consumer,
    ...
    protected void buildRecipes(@NotNull final Consumer<FinishedRecipe> consumer) {
        ILikeWood.WOOD_TYPE_REGISTRY.getWoodTypes().forEach(woodType ->
        {
            ...
            if (ILikeWood.WOODEN_RESOURCE_REGISTRY.hasLog(woodType)) {
                final var log = Objects.requireNonNull(ForgeRegistries.BLOCKS.getValue(ILikeWood.WOODEN_RESOURCE_REGISTRY.getLog(woodType).getResource()));
                ...
                if (ILikeWood.WOODEN_RESOURCE_REGISTRY.hasStrippedLog(woodType)) {
                    final var strippedLog = Objects.requireNonNull(ForgeRegistries.BLOCKS.getValue(ILikeWood.WOODEN_RESOURCE_REGISTRY.getStrippedLog(woodType).getResource()));
                    sawmillingRecipe(Ingredient.of(log), strippedLog).unlockedBy(String.format("has_%s", logPath), has(log)
                    ).save(consumer,
                        new ResourceLocation(Constants.MOD_ID,
                            Util.toRegistryName(Objects.requireNonNull(ForgeRegistries.BLOCKS.getKey(strippedLog)).getPath(), "from", logPath, "sawmilling"))
                    );
                }
            }
            if (ILikeWood.WOODEN_RESOURCE_REGISTRY.hasStrippedLog(woodType)) {
