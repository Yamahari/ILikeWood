...
public final class PanelsSlabRecipeProvider extends AbstractBlockItemRecipeProvider {
    ...
    protected void registerRecipes(@Nonnull final Consumer<FinishedRecipe> consumer, final IWoodType woodType, final Block block) {
        ...
        if (ILikeWood.WOODEN_RESOURCE_REGISTRY.hasPlanks(woodType)) {
            final ItemLike planks = ForgeRegistries.BLOCKS.getValue(ILikeWood.WOODEN_RESOURCE_REGISTRY.getPlanks(woodType).getResource());

            ShapedRecipeBuilder.shaped(RecipeCategory.BUILDING_BLOCKS, Objects.requireNonNull(planks))
                .define('S', block)
                .pattern("S")
                .pattern("S")
                ...
                .save(consumer,
                    new ResourceLocation(Constants.MOD_ID,
                        Util.toRegistryName(Objects.requireNonNull(ForgeRegistries.ITEMS.getKey(planks.asItem())).getPath(),
                            "from",
