...
public final class NetheriteTieredItemRecipeProvider extends RecipeProvider {
    ...
    @Override
    protected void buildRecipes(@Nonnull final Consumer<FinishedRecipe> consumer) {
        ILikeWood.TIERED_ITEM_REGISTRY.getObjects(tieredItemType).filter(tieredItem -> ((IWoodenTieredItem) tieredItem).getWoodenItemTier().equals(VanillaWoodenItemTiers.DIAMOND))
    ...
    private void registerRecipes(@Nonnull final Consumer<FinishedRecipe> consumer, final Item item) {
        ...
        try {
            final Item output = ILikeWood.TIERED_ITEM_REGISTRY.getObject(VanillaWoodenItemTiers.NETHERITE, woodType, tieredItem.getTieredItemType());

            SmithingTransformRecipeBuilder.smithing(Ingredient.EMPTY, Ingredient.of(item), Ingredient.of(Items.NETHERITE_INGOT), RecipeCategory.TOOLS, output)
                .unlocks("has_netherite_ingot", has(Items.NETHERITE_INGOT))
                .save(consumer, new ResourceLocation(Constants.MOD_ID, Util.toRegistryName(Objects.requireNonNull(ForgeRegistries.ITEMS.getKey(output)).getPath(), "smithing")));
